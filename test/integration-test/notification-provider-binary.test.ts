    ...
    const server = Bun.listen({
        ...
        socket: {
            open(socket) {
                ...
                socket.write("220 iglo-monitor-test ESMTP\r\n");
            },
            data(socket, chunk) {
                ...
                while ((lineEnd = connection.buffer.indexOf("\r\n")) !== -1) {
                    ...
                    if (/^EHLO\b/i.test(command)) {
                        socket.write("250-iglo-monitor-test\r\n250 PIPELINING\r\n");
...
async function waitForApp(url) {
    ...
    while (Date.now() < deadline) {
        if (appProcess.exitCode !== null) {
            ...
            throw new Error(
                `iglo.monitor exited before becoming ready (exit ${appProcess.exitCode})${stderr ? `: ${stderr.trim()}` : ""}`
            );
        }
        try {
...
async function stopApp() {
    ...
    try {
        await withTimeout(appProcess.exited, 5_000, "iglo.monitor did not stop after SIGTERM");
    } catch {
...
describe("compiled runtime loading", () => {
    test("compiled executable runs SMTP provider and monitor factories through production flows", async () => {
        expect(binaryPath, "IGLO_MONITOR_BINARY must point to a compiled iglo.monitor executable").toBeTruthy();
        expect(fs.existsSync(binaryPath)).toBe(true);

        dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "iglo-monitor-compiled-smtp-"));
    ...
    test("standalone executable serves embedded assets and records a ping heartbeat", async () => {
        expect(binaryPath, "IGLO_MONITOR_BINARY must point to a compiled iglo.monitor executable").toBeTruthy();
        dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "iglo-monitor-compiled-ping-"));
        standaloneDir = fs.mkdtempSync(path.join(os.tmpdir(), "iglo-monitor-compiled-standalone-"));
